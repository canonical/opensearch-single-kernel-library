...
def cert_expiration_remaining_hours(cert: str) -> int:
    ...
    time_difference = certificate_object.not_valid_after.replace(
        tzinfo=timezone.utc
...
def _parse_ca_alias(alias: str) -> tuple[str, int]:
    """Return a base alias and optional chain index."""
...
def _ca_entry_from_block(block: str) -> tuple[str, int, str] | None:
    ...
    base, idx = _parse_ca_alias(alias)
    pem = f"{start_cert_marker}{block.split(start_cert_marker, 1)[1]}".strip()
    return base, idx, pem


def _list_ca_chains(stored_certs: str) -> dict[str, list[tuple[int, str]]]:
    """Group stored CA certificates by base alias."""
    chains: dict[str, list[tuple[int, str]]] = {}
    for block in split_ca_chain(stored_certs):
        if entry := _ca_entry_from_block(block):
            base, idx, pem = entry
...
def _assemble_ca_chains(chains: dict[str, list[tuple[int, str]]]) -> dict[str, str]:
    """Reassemble CA chains in index order."""
    out: dict[str, str] = {}
