...
class ClusterManager(BaseManager):
    ...
    def reconcile_cluster_config(self) -> bool:
        ...
        if not (current_deployment_desc := self.state.application.deployment_description):
            # new cluster
    ...
    def reconcile_cluster_config_with_relation_data(self, data: PeerClusterAppModel) -> None:  # noqa: C901
        ...
        current_deployment_desc = self.state.application.deployment_description
        ...
        new_deployment_desc = DeploymentDescription(
            ...
            cluster_name_autogenerated=data.deployment_description.cluster_name_autogenerated,
        )
    ...
    def should_initialise_security_index(self) -> bool:
        ...
        return not self.state.application.security_index_initialised and (
            "data" in self.state.application.deployment_description.config.roles
            or self.state.application.deployment_description.start
    ...
    def initialise_security_index(self) -> None:
        ...
        args = [
            ...
            f"-cn {self.state.application.deployment_description.config.cluster_name}",
    ...
    def no_blocking_directives(self, deployment_desc: DeploymentDescription | None = None) -> bool:
        ...
        if not (
            deployment_desc := deployment_desc or self.state.application.deployment_description
        ):
            return False
    ...
    def get_nodes(self, use_localhost: bool) -> list[Node]:
        ...
        if self.state.planned_units == 0 and not self.state.application.deployment_description:
            # This app is going away and the -broken event already happened
            return []

        # This means it's the first unit on the cluster.
        if self.state.application.deployment_description.start == StartMode.WITH_PROVIDED_ROLES:
            computed_roles = self.state.application.deployment_description.config.roles
        else:
    ...
    def configure_bootstrap_contributors(
        ...
        if "cluster_manager" in computed_roles:
            ...
            if (
                self.state.application.deployment_description.typ
    ...
    def cleanup_on_last_unit_removal(self) -> None:
        ...
        if self.state.peer_relation:
            self.state.application.reset("bootstrap_contributors_count", "nodes_config")
            ...
            if not self.state.application.is_data_role_in_cluster_fleet_apps:
                self.state.application.reset("security_index_initialised", "bootstrapped")
    ...
    def no_cluster_manager_left(self) -> bool:
        ...
        return (
            len(
                [
                    app
                    for app in self.state.application.cluster_fleet_apps.values()
                    if app.app.id != self.state.application.deployment_description.app.id
                ]
            )
            > 0
