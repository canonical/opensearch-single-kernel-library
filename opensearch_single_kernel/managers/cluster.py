...
class ClusterManager(BaseManager):
    ...
    def _existing_cluster_setup(
        ...
        deployment_desc = (
            prev_deployment_desc.copy()
        )  # avoid mutating the previous deployment description in state
        directives = deployment_desc.pending_directives
        deployment_state = deployment_desc.state
        try:
            self._pre_validate_roles_change(
                new_roles=config.roles, prev_roles=deployment_desc.config.roles
            )
            if deployment_desc.state.value == State.BLOCKED_CANNOT_APPLY_NEW_ROLES:
        ...
        if (
            ...
            and deployment_desc.state.value == State.BLOCKED_CANNOT_START_WITH_ROLES
        ...
        dep_type = deployment_type(config, start_mode, deployment_desc.typ)
        return DeploymentDescription(
            app=deployment_desc.app,
            config=PeerClusterConfig(
                cluster_name=deployment_desc.config.cluster_name,
                init_hold=deployment_desc.config.init_hold,
                roles=config.roles,
                data_temperature=config.data_temperature,
            ),
            start=start_mode,
            ...
            cluster_name_autogenerated=deployment_desc.cluster_name_autogenerated,
            promotion_time=(
                deployment_desc.promotion_time
    ...
    def flush_translog_to_disk(self) -> None:
        ...
        if self.opensearch_client.is_node_up():
            try:
                self.opensearch_client.flush_opensearch_translog(self.alt_hosts)
            except OpenSearchHttpError:
