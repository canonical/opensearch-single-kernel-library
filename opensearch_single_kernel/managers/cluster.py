...
class ClusterManager(BaseManager):
    ...
    def start(self, wait_until_http_200: bool = True) -> None:
        ...
        start = datetime.now()
        while (
            not (connected := _is_connected())
            and (datetime.now() - start).seconds < 180
        ):
            time.sleep(3)
        if not connected:
            logger.debug(
                "Waited %s but OpenSearch did not start", datetime.now() - start
            )
    ...
    def reconcile_cluster_config(self) -> bool:
        ...
        if not (current_deployment_desc := self.state.application.deployment_desc):
            ...
            logger.debug(
                "New deployment_desc from new cluster setup: %s", deployment_desc
            )
            self.state.application.deployment_desc = deployment_desc
            return False
        # update cluster deployment desc
        logger.debug(
            "Existing deployment_desc before cluster setup: %s", current_deployment_desc
        )
        deployment_desc = self._existing_cluster_setup(
            user_config, current_deployment_desc
        )
        logger.debug(
            "Existing deployment_desc after cluster setup: %s", deployment_desc
        )
    ...
    def reconcile_cluster_config_with_relation_data(
        ...
        self.state.application.nodes_config = {
            node.name: node for node in data.cm_nodes
        }
    ...
    def _new_cluster_setup(self, config: PeerClusterConfig) -> DeploymentDescription:
        ...
        if config.init_hold:
            ...
            start_mode = (
                StartMode.WITH_PROVIDED_ROLES
                if config.roles
                else StartMode.WITH_GENERATED_ROLES
            )
            return DeploymentDescription(
                app=App(
                    model_uuid=self.state.model_uuid, name=self.state.application.name
                ),
                config=config,
                start=start_mode,
        ...
        if not (cluster_name := config.cluster_name.strip()):
            cluster_name = (
                f"{self.state.application.name}-{ShortUUID().random(length=4)}".lower()
            )
    ...
    def _existing_cluster_setup(
        ...
        if (
            ...
            and prev_deployment_desc.state.value
            == State.BLOCKED_CANNOT_START_WITH_ROLES
            and (
                start_mode == StartMode.WITH_GENERATED_ROLES
    ...
    def _pre_validate_roles_change(
        self, new_roles: list[str], prev_roles: list[str]
    ) -> None:
        ...
        if "cluster_manager" in new_roles and "voting_only" in new_roles:
            ...
            raise OpenSearchProvidedRolesException(
                CLUSTER_MANAGER_VOTING_ROLES_PROVIDED_INVALID
            )
    ...
    def should_initialise_security_index(self) -> bool:
        ...
        return not self.state.application.is_security_index_initialised and (
            ...
            or self.state.application.deployment_desc.start
    ...
    def assert_current_node_joined_cluster(self) -> None:
        ...
        for node in nodes:
            ...
            raise OpenSearchNotFullyReadyError(
                "Node online but not in cluster (expected node.name=%s)."
        ...
        for attempt in Retrying(
            ...
            with attempt:
                if not self.opensearch_client.is_node_up():
                    raise OpenSearchNotFullyReadyError(
                        "Node started but not fully ready yet."
                    )

    def no_blocking_directives(
        self, deployment_desc: DeploymentDescription | None = None
    ) -> bool:
        """Return If we have any blocking directives."""
        if not (
            deployment_desc := deployment_desc or self.state.application.deployment_desc
        ):
            return False
    ...
    def get_nodes(self, use_localhost: bool) -> list[Node]:
        ...
        if (
            self.state.application.deployment_desc.start
            == StartMode.WITH_PROVIDED_ROLES
        ):
    ...
    def clear_directive(self, directive: Directive) -> None:
        ...
        logger.debug(
            "Clearing directive %s. DeploymentDesc: %s", directive, deployment_desc
        )
        self.state.application.deployment_desc = deployment_desc
    ...
    def configure_bootstrap_contributors(
        ...
        if "cluster_manager" in computed_roles:
            ...
            if (
                self.state.application.deployment_desc.typ
                ...
                if cms_in_bootstrap < self.state.planned_units:
                    ...
                    if self.state.server.is_app_leader:
                        self.state.application.bootstrap_contributors_count = (
                            cms_in_bootstrap + 1
                        )
    ...
    def is_opensearch_started(self) -> bool:
        ...
        reachable = self.workload.is_reachable(
            self.state.node_host, OPENSEARCH_HTTP_PORT
        )
        if not reachable:
    ...
    def reset_nodes_conf_to_default(
        self, app_id: str, nodes: list[Node]
    ) -> dict[str, Node]:
        """Recompute the configuration of all the nodes (cluster set to auto-generate roles)."""
        if not nodes:
            return {}
        logger.debug(
            "Roles before re-balancing: %s", {node.name: node.roles for node in nodes}
        )
        nodes_by_name = {}
    ...
    def promote_deployment_type(self) -> None:
        ...
        if (
            Directive.WAIT_FOR_PEER_CLUSTER_RELATION
            in deployment_desc.pending_directives
        ):
            deployment_desc.pending_directives.remove(
                Directive.WAIT_FOR_PEER_CLUSTER_RELATION
            )
    ...
    def _add_app_statuses(self, status_list: list[StatusObject]) -> None:  # noqa: C901
        ...
        if (
            (config_roles := self.state.config.get("roles"))
            and isinstance(config_roles, str)
            and "cluster_manager" in config_roles
            and "voting_only" in config_roles
        ):
            status_list.append(
                GeneralStatuses.CLUSTER_MANAGER_VOTING_ONLY_INVALID.value
            )
