...
class PeerClusterOrchestratorManager(BaseManager):
    ...
    def refresh_relation_data(  # noqa: C901
        ...
        if (
            self.set_peer_cluster_err_data_if_wrong_integration(
                event_rel_id, rel_err_data
            )
            and event_rel_id
        ):
            return True
        ...
        for local_peer_cluster in self.state.peer_clusters(
            is_provider=True, remote=False
        ):
            ...
            orchestrators.update(
                {
                    ...
                    f"{cluster_type}_rel_id": local_peer_cluster.relation.id
                    if has_units
                    else -1,
                }
            )
    ...
    def build_peer_cluster_rel_data(
        ...
        try:
            ...
            logger.warning(
                f"Could not fetch nodes in related {deployment_desc.typ} sub-cluster"
            )
    ...
    def fetch_current_app_cm_nodes(
        ...
        if cluster_fleet_apps := self.state.application.cluster_fleet_apps:
            ...
            has_planned_units = lambda app_id: (
                app_id in cluster_fleet_apps
    ...
    def build_peer_cluster_rel_data_credentials(
        ...
        if self.state.application.is_admin_user_initialized:
            return PeerClusterRelDataCredentials(
                ...
                admin_password=self.state.secrets.get(
                    Scope.APP, password_key(ADMIN_USER)
                ),
                admin_password_hash=self.state.secrets.get(
                ...
                monitor_password=self.state.secrets.get(
                    Scope.APP, password_key(COS_USER)
                ),
                admin_tls=self.state.secrets.get_object(
    ...
    def plugin_config_info(self) -> dict[str, PluginConfigInfo]:
        ...
        return {
            ...
            if plugin.secret_id
            and self.state.secrets.grant_secret_to_subclusters(
                plugin.secret_id, is_provider=True
            )
        }
    ...
    def build_peer_cluster_rel_err_data(  # noqa: C901
        ...
        if not deployment_desc:
            blocked_msg = PeerClusterErrorDataStatuses.MAIN_OR_FAILOVER_NOT_CONFIGURED.value.message
        elif deployment_desc.typ == DeploymentType.OTHER:
            should_sever_relation, should_retry = True, False
            blocked_msg = PeerClusterErrorDataStatuses.RELATED_TO_NON_MAIN_OR_FAILOVER.value.message
        elif (
            ...
            should_sever_relation, should_retry = True, False
            blocked_msg = (
                PeerClusterErrorDataStatuses.CANNOT_HAVE_TWO_FAILOVERS.value.message
            )
        elif not self.state.application.is_admin_user_initialized:
            blocked_msg = PeerClusterErrorDataStatuses.ADMIN_USER_NOT_FULLY_CONFIGURED.value.message.format(
                message_suffix=message_suffix
            )
        elif not self.state.is_tls_full_configured_in_cluster:
            blocked_msg = PeerClusterErrorDataStatuses.TLS_NOT_FULLY_CONFIGURED.value.message.format(
                message_suffix=message_suffix
            )
            should_retry = False
        elif (
            ...
            if not self.is_every_unit_marked_as_started:
                ...
                try:
                    ...
                    logger.error(e)
                    blocked_msg = PeerClusterErrorDataStatuses.COULD_NOT_FETCH_NODES.value.message.format(
                        message_suffix=message_suffix
                    )
