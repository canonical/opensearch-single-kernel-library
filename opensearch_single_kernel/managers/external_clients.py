...
class ExternalClientsManager(BaseManager):
    ...
    def create_opensearch_users(
        ...
        if extra_user_roles == KIBANA_SERVER_ROLE:
            ...
            pwd = self.state.secrets.get(Scope.APP, self.state.secrets.password_key(username))
        else:
    ...
    def update_relation_version(self, external_client: ExternalOpenSearchClient):
        """Update the relation databag with version."""
        external_client.version = self.version

    def update_relation_index(self, external_client: ExternalOpenSearchClient, index: str):
        """Update the relation databag with index."""
    ...
    def remove_lingering_relation_users_and_roles(  # noqa: C901
        ...
        if departed_external_client and (
    ...
    def update_dashboards_password(self):
        ...
        pwd = self.state.secrets.get(
            Scope.APP, self.state.secrets.password_key(KIBANA_SERVER_USER)
        )
        for dashboards_client in self.state.dashboards_clients:
            self.update_relation_credentials(dashboards_client, KIBANA_SERVER_USER, pwd)
