...
async def get_elected_cm_unit_id(juju: jubilant.Juju, unit_ip: str, app: str = APP_NAME) -> int:
    ...
    # get all nodes
    resp = await http_request(juju, "GET", f"https://{unit_ip}:9200/_nodes", app=app)
...
async def get_number_of_shards_by_node(
    ...
    init_cluster_alloc = await cluster_allocation(juju, unit_ip, app=app)
...
async def all_nodes(juju: jubilant.Juju, unit_ip: str, app: str = APP_NAME) -> list[Node]:
...
async def get_shards_by_state(juju: jubilant.Juju, unit_ip: str) -> dict[str, list[str]]:
...
def storage_app_entries(juju: jubilant.Juju, app: str) -> list[str]:
...
def storage_type(juju: jubilant.Juju, app: str) -> str | None:
    """Retrieves type of storage associated with an application."""
    storage_entries = storage_app_entries(juju, app)
...
def storage_id(juju: jubilant.Juju, app: str, unit_id: int):
...
async def all_processes_down(juju: jubilant.Juju, app: str) -> bool:
    """Check if all processes are down."""
    for unit_id in get_application_unit_ids(juju, app):
        unit_name = f"{app}/{unit_id}"
        try:
            opensearch_pid = juju.cli(
                "exec", "--unit", unit_name, "--", "sudo", "lsof", "-ti:9200"
            )
...
async def send_kill_signal_to_process(  # noqa: C901
    juju: jubilant.Juju,
    app: str,
    unit_id: int,
    signal: str,
    ...
    if opensearch_pid is None:
        if substrate == "vm":
            try:
                opensearch_pid = juju.cli("ssh", unit_name, "--", "sudo", "lsof", "-ti:9200")
            except jubilant.CLIError:
                opensearch_pid = ""
        else:
            try:
                opensearch_pid = juju.cli(
                    "ssh", "--container", "opensearch", unit_name, "lsof", "-ti:9200"
                )
            except jubilant.CLIError:
                opensearch_pid = ""
    ...
    if substrate == "vm":
        try:
            juju.cli("ssh", unit_name, "--", "sudo", "kill", f"-{signal.upper()}", opensearch_pid)
        except jubilant.CLIError as e:
            raise Exception(f"kill failed: {e}")
    else:
...
async def update_restart_delay(juju: jubilant.Juju, app: str, unit_id: int, delay: int):
    ...
    replace_delay_cmd = (
        ...
        "-i",
        "-e",
        f"s/^RestartSec=[0-9]\\+/RestartSec={delay}/g",
        OPENSEARCH_SERVICE_PATH,
    )
    juju.cli(*replace_delay_cmd)

    # reload the daemon for systemd to reflect changes
    reload_cmd = ("exec", "--unit", unit_name, "--", "sudo", "systemctl", "daemon-reload")
    juju.cli(*reload_cmd)


async def cut_network_from_unit_with_ip_change(
    juju: jubilant.Juju, app: str, unit_id: int
) -> None:
...
async def is_network_restored_after_ip_change(
    juju: jubilant.Juju, app: str, unit_id: int, unit_ip: str, retries: int = 50
) -> bool:
    try:
...
async def add_juju_secret(
    juju: jubilant.Juju, charm_name: str, secret_label: str, data: dict[str, str]
) -> str:
    ...
    try:
        stdout = juju.cli("add-secret", secret_label, *kv_args)
    except jubilant.CLIError as e:
        raise AssertionError(f"juju add-secret failed: {e}")
    ...
    try:
        juju.cli("grant-secret", secret_uri, charm_name)
    except jubilant.CLIError as e:
        raise AssertionError(f"juju grant-secret failed: {e}")

    return secret_uri


async def wait_for_backup_system_to_settle(
    juju: jubilant.Juju, leader_id: int, unit_ip: str, app: str = APP_NAME
):
    ...
    for attempt in Retrying(stop=stop_after_attempt(8), wait=wait_fixed(15)):
        with attempt:
            ...
            action = await run_action(
                juju, leader_id, "list-backups", params={"output": "json"}, app=app
            )
            # Expected format:
...
async def assert_start_and_check_continuous_writes(
    juju: jubilant.Juju, unit_ip: str, app: str
) -> None:
    ...
    """
    initial_count = await index_docs_count(juju, app, unit_ip, ContinuousWrites.INDEX_NAME)
    logger.info(
        f"Index {ContinuousWrites.INDEX_NAME} has {initial_count} documents, starting there"
    )
    writer = ContinuousWrites(juju, app, initial_count=initial_count)
    await writer.start()
    time.sleep(10)
...
async def create_backup(
    ...
    action = await run_action(juju, leader_id, "create-backup", app=app)
    logger.debug(f"create-backup output: {action}")
    await wait_for_backup_system_to_settle(juju, leader_id, unit_ip, app=app)
...
async def restore(
    juju: jubilant.Juju, backup_id: str, unit_ip: str, leader_id: int, app: str = APP_NAME
) -> bool:
    """Restores a backup."""
    action = await run_action(juju, leader_id, "restore", params={"backup-id": backup_id}, app=app)
...
async def list_backups(juju: jubilant.Juju, leader_id: int, app: str = APP_NAME) -> dict[str, str]:
    action = await run_action(juju, leader_id, "list-backups", params={"output": "json"}, app=app)
    assert action.status == "completed"
    return json.loads(action.response["backups"])


async def assert_restore_indices_and_compare_consistency(
    juju: jubilant.Juju, app: str, leader_id: int, unit_ip: str, backup_id: str
) -> None:
    """Ensures that continuous writes index has at least the value below."""
    original_count = await index_docs_count(juju, app, unit_ip, ContinuousWrites.INDEX_NAME)
    ...
    new_count = await index_docs_count(juju, app, unit_ip, ContinuousWrites.INDEX_NAME)
    logger.info(
